...
class RoBERTaClassifier(BaseModel):
    def __init__(
        ...
        self.config.extra_layers = self.extra_layers
        self.config.dropout_rate = self.dropout_rate
        self.config.class_weights = (
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = self._RoBERTaClassifier(
            model_name, self.config, dropout_rate, self.extra_layers, class_weights
        )
        self.trainer = None

    class _RoBERTaClassifier(nn.Module):
        def __init__(self, model_name, config, dropout_rate, extra_layers, class_weights):
            super().__init__()
            self.transformer = AutoModel.from_pretrained(model_name, config=config)
            self.pre_classifier = nn.Linear(config.hidden_size, config.hidden_size)
            self.pre_classifier_act = nn.ReLU()
